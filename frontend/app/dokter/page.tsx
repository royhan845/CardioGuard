...
export default function DokterPage() {
  ...
  const handleSearchTicket = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/ticket/${ticketInput.toUpperCase()}`);
      ...
      if (data.status === 'success') {
        ...
        alert(data.message || "Tiket tidak ditemukan di sistem.");
        setPatientData(null);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!patientData) {
      alert("Silakan cari data pasien terlebih dahulu menggunakan ID Tiket.");
      return;
    }
    ...
    try {
      ...
      alert("Gagal memproses prediksi Machine Learning.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-10 print:hidden">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-500 mt-1">Sistem Evaluasi Hibrida: NLP & Machine Learning</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 print:block">
          ...
          <div className="lg:col-span-2 space-y-6 print:hidden">
            ...
            {/* 2. DATA PASIEN & HASIL NLP */}
            {patientData && (
              <div className="bg-teal-50 p-8 rounded-3xl shadow-sm border border-teal-100 animate-in fade-in zoom-in duration-300">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <span className="text-xs font-bold text-teal-600 uppercase tracking-widest">Informasi Pasien</span>
                    <h3 className="text-2xl font-bold text-teal-900 mt-1">{patientData.nama_pasien}</h3>
                    <p className="text-sm font-mono text-teal-700 mt-1 bg-teal-100 px-2 py-1 rounded inline-block">ID: {patientData.ticket_code}</p>
                  </div>
                  <span className={`text-xs font-bold px-3 py-1 rounded-full ${patientData.status_pemeriksaan === 'Selesai' ? 'bg-emerald-200 text-emerald-800' : 'bg-amber-200 text-amber-800'}`}>
                    Status: {patientData.status_pemeriksaan}
                  </span>
                </div>
                
                <div className="bg-white p-5 rounded-2xl border border-teal-100">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Riwayat Keluhan Teks (Mentah)</p>
                  <p className="text-slate-700 text-sm italic">"{patientData.keluhan_teks}"</p>
            ...
            {/* 3. FORM 13 PARAMETER ML */}
            <div className={`bg-white p-8 rounded-3xl shadow-sm border border-slate-200 transition-opacity duration-300 ${!patientData ? 'opacity-50 pointer-events-none' : ''}`}>
              <div className="flex justify-between items-center border-b border-slate-100 pb-4 mb-6">
                <h2 className="text-lg font-bold text-slate-800">Input Data Lab Medis (Tabular)</h2>
                {!patientData && <span className="text-xs font-medium text-red-500 bg-red-50 px-3 py-1 rounded-full">Terkunci - Cari Pasien Dulu</span>}
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <button type="submit" disabled={loading} className="w-full flex justify-center items-center bg-teal-700 hover:bg-teal-800 text-white font-semibold py-4 rounded-xl transition-all shadow-sm mt-4 disabled:opacity-70">
                  {loading ? 'Mengkalkulasi...' : 'Eksekusi Kalkulasi ML'}
                </button>
              </form>
            </div>
          </div>
          ...
          <div className="lg:col-span-1 print:w-full print:block">
            {result && patientData ? (
              <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200 animate-in fade-in slide-in-from-bottom-4 print:shadow-none print:border-none print:p-0">
                ...
                <div className="hidden print:block mb-8">
                  ...
                  <div className="text-sm text-slate-700 mb-6 space-y-2">
                    <p><strong>ID Tiket:</strong> {patientData.ticket_code}</p>
                    <p><strong>Nama Pasien:</strong> {patientData.nama_pasien}</p>
                    <p><strong>Tanggal Keluar:</strong> {new Date().toLocaleDateString('id-ID')}</p>
                  ...
                  <h2 className="text-sm font-bold text-slate-800 uppercase tracking-widest border-b border-slate-300 pb-2 mb-4">B. Parameter Klinis (Tabular)</h2>
                ...
                <h2 className="text-lg font-bold text-slate-800 mb-4 print:text-sm print:uppercase print:tracking-widest print:border-b print:border-slate-300 print:pb-2">C. Keputusan Akhir Machine Learning</h2>
              ...
              <div className="bg-white p-8 rounded-3xl border border-dashed border-slate-200 h-full flex flex-col items-center justify-center text-slate-400 text-center print:hidden">
                ...
                <p className="text-sm font-medium text-slate-500">Panel Hasil Analisis<br/><span className="text-xs font-normal mt-1 block">Silakan cari tiket pasien dan eksekusi model ML.</span></p>
              </div>
            )}
          </div>
